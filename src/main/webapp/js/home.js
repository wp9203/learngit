layui.define(['element','layer', 'http'],function(exports){
    ...
    var headMenus = ['views/user-info.html','views/password-modify.html'];
    ...
    var Home = function(){
        ...
        $('#head-menu a').on('click', function (event) {
            // operateType = "0";
            event.preventDefault();
            var $this = $(this), url = $this.attr('href'),
                title = $.trim($this.text());
            if (url == 'login.html') {
                //退出登录
                http.post('api/login/logout', {}, function (data) {
        ...
        tabs.onChange(function(data){
            ...
            if($this && typeof $this === 'object') {
                ...
                var url = $(this)[0].getAttribute('lay-id');
                if (headMenus.indexOf(url) == -1) {
